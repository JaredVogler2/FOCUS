...
class ProductionScheduler:
    ...
    def _load_late_parts_and_rework(self, sections):
        """Load late parts and rework tasks with string task IDs"""
        ...
        if "LATE PARTS RELATIONSHIPS TABLE" in sections:
            ...
            for _, row in df.iterrows():
                try:
                    ...
                    # Store on_dock_dates with the actual task ID
        ...
        if "LATE PARTS TASK DETAILS" in sections:
            ...
            for _, row in df.iterrows():
                try:
                    # Task ID is already prefixed in CSV
                    ...
                    # Use task_id directly as instance_id since it's already prefixed
                    instance_id = task_id
        ...
        if "REWORK TASK DETAILS" in sections:
            ...
            for _, row in df.iterrows():
                try:
                    ...
                    instance_id = task_id  # Use directly since it's already prefixed
                    ...
                    if needs_qi.lower() in ['yes', 'y', '1', 'true']:
                        # Create quality inspection with QI_ prefix
                        qi_instance_id = f"QI_{task_id}"  # e.g., QI_RW_401
    ...
    def build_dynamic_dependencies(self):
        ...
        for lp_constraint in self.late_part_constraints:
            ...
            # First task - check if it's already a prefixed string or needs conversion
            if first_task in self.tasks:
                # It's already a valid task instance ID (e.g., "LP_1001")
                first_instance = first_task
            elif str(first_task).startswith('LP_'):
                # It has the prefix but might not be in tasks
                first_instance = first_task if first_task in self.tasks else None
            else:
                # Legacy numeric ID - add prefix
                first_instance = f"LP_{first_task}" if f"LP_{first_task}" in self.tasks else None

            # Second task - could be prefixed, numeric baseline, or another special task
            second_instance = None
            if second_task in self.tasks:
                # It's already a valid task instance ID
                ...
                # It's a numeric ID - could be baseline or need prefix
                task_num = int(second_task)
                if task_num < 1000:
                    # It's a baseline task
                    second_instance = self.task_instance_map.get((product, task_num)) if product else None
                    # If no product specified, try to find it in any product
                    ...
                    # Check if it needs LP_ or RW_ prefix
        ...
        for rw_constraint in self.rework_constraints:
            ...
            # First task - check if it's already prefixed or needs conversion
            if first_task in self.tasks:
                # It's already a valid task instance ID (e.g., "RW_401")
                first_instance = first_task
            ...
            if second_task in self.tasks:
                ...
                if task_num < 1000:
                    ...
                    # Check if it needs RW_ or LP_ prefix
        ...
        # 4. Add remaining QI constraints (for any QI relationships not already added)
                ...
                if self.is_working_day(test_time, product):
                    ...
                    try:
                        ...
                        if valid:
                            ...
                            day_util = self.calculate_day_utilization(team, potential_start.date())
                            week_util = self.calculate_week_utilization(team, potential_start.date())
                            ...
                            gap_penalty = self.calculate_gap_penalty(team, potential_start, test_end) * aggressiveness
    ...
    def calculate_day_utilization(self, team, target_date):
        ...
        available_minutes = 8 * 60 * capacity  # 8 hours per day per person
        if available_minutes > 0:
            return (total_minutes / available_minutes) * 100
        return 0
    ...
    def get_next_working_time_with_capacity_with_changeover(self, current_time, product_line, team,
                                                            mechanics_needed, duration, is_quality=False):
        """
        Get next available working time with capacity, accounting for product changeover
        """
        # Check if changeover is needed
        needs_changeover, last_product = self.needs_product_changeover(team, product_line, current_time)

        if needs_changeover:
            # Add 15 minute changeover time
            current_time = current_time + timedelta(minutes=15)
            if self.debug:
                print(f"  [CHANGEOVER] {team} switching from {last_product} to {product_line} (+15 min)")

        # Now find next available slot with the regular method
        return self.get_next_working_time_with_capacity(
            current_time, product_line, team, mechanics_needed, duration, is_quality
        )
    ...
    def schedule_tasks_with_critical_path_awareness(self, safety_buffer_days=2, silent_mode=False):
        ...
        # Tasks with no constraints at all
        orphaned_tasks = all_tasks - tasks_with_incoming_constraints - tasks_with_outgoing_constraints
        ...
        # Add tasks that have only outgoing constraints
        tasks_with_only_outgoing = tasks_with_outgoing_constraints - tasks_with_incoming_constraints
        for task in tasks_with_only_outgoing:
            priority = self.calculate_task_priority(task)
            heapq.heappush(ready_tasks, (priority, task))
        ...
        while ready_tasks and scheduled_count < total_tasks and iteration_count < max_iterations:
            ...
            try:
                if criticality == 'CRITICAL':
                    ...
                    # Limited spreading - look ahead up to 2 days
                    buffer_count += 1
                    ...
                    # Can spread more aggressively - look ahead up to 5 days
                    flexible_count += 1
                    scheduled_start, shift = self.find_best_slot_within_window(
                        task_instance_id, earliest_start,
                ...
                self.task_schedule[task_instance_id] = {
                    ...
                    'task_type': task_type,
                    'shift': shift,
                    'criticality': criticality,  # Store for visibility
                    'original_task_id': self.instance_to_original_task.get(task_instance_id)
                }
    ...
    def find_best_slot_within_window(self, task_instance_id, earliest_start,
        ...
        # Calculate safe window based on delivery and buffers
        ...
        slots_evaluated = 0
        while test_time < end_window and slots_evaluated < 50:  # Limit evaluations
            if self.is_working_day(test_time, product):
                try:
                    # Check if slot is available
                    slot_start, shift = self.get_next_working_time_with_capacity(
                        test_time, product, team,
                        mechanics_needed, duration, is_quality
                    )
                    ...
                    if slot_start < end_window:
                        ...
                        # Scoring factors:
                        # - Prefer low-utilization days (spread work)
                        # - But not too far from earliest (maintain flow)
                        delay_days = (slot_start - earliest_start).total_seconds() / 86400

                        # Score calculation
                        utilization_score = (100 - day_util) * 2  # Prefer empty days
                        delay_penalty = delay_days * 20  # Penalty for delay
                    ...
                    pass  # Skip if can't schedule at this time

            # Move to next test time
            test_time += timedelta(hours=4)  # Check every 4 hours
    ...
    def scenario_3_multidimensional_optimization(self, target_lateness=-1, scenario2_results=None, max_iterations=300,
        ...
        """
        Scenario 3: Find MINIMUM workforce to achieve TARGET lateness with critical-path-aware scheduling
        Phase 1: Find minimum workforce that achieves target lateness
        Phase 2: Optimize workforce while maintaining target
        Phase 3: Re-schedule with critical-path awareness for better utilization
        """
        print("\n" + "=" * 80)
        ...
        if enable_critical_path_scheduling:
            print("Critical-path scheduling: ENABLED")
        print("=" * 80)
        ...
        # Calculate minimum requirements based on task needs
        min_requirements = self.calculate_minimum_team_requirements()
        ...
        # Set upper bounds based on Scenario 2 results if available
        ...
        # Initialize configuration with minimum requirements
        current_mech_config = {}
        current_qual_config = {}
        ...
        best_variance = None  # Initialize for all cases
        ...
        while iteration < max_iterations // 2 and not achieved_target:
            ...
            try:
                ...
                if max_lateness <= target_lateness:
                    ...
                    # Increase capacity based on gap to target
                    ...
                    # Try increasing mechanic capacity first
                    ...
                    if not team_increased:
                        # If all mechanic teams at max, try quality teams
        ...
        if achieved_target and best_config:
            ...
            # Try to reduce workforce while maintaining target
            ...
            # PHASE 3: Critical-Path-Aware Optimization
            if enable_critical_path_scheduling:
                print("\n" + "=" * 80)
                print("PHASE 3: Critical-Path-Aware Scheduling Optimization")
                print("-" * 80)
                print("Attempting to improve utilization while respecting task criticality...")
                ...
                # First, get baseline with standard ASAP scheduling
                self.task_schedule = {}
                self._critical_path_cache = {}
                self.generate_global_priority_list(allow_late_delivery=True, silent_mode=True)

                baseline_metrics = self.calculate_lateness_metrics()
                baseline_max_lateness = max((m['lateness_days'] for m in baseline_metrics.values()
                                             if m['lateness_days'] < 999999), default=0)
                baseline_variance = self.calculate_utilization_variance()

                print(f"\n  Baseline (ASAP) scheduling:")
                print(f"    Max lateness: {baseline_max_lateness} days")
                print(f"    Utilization variance: {baseline_variance:.2f}")

                # Save baseline as best
                best_schedule = self.task_schedule.copy()
                best_variance = baseline_variance
                best_lateness = baseline_max_lateness
                best_safety_buffer = 0

                # Try different safety buffer settings for critical path scheduling
                for safety_buffer in [1, 2, 3]:
                    print(f"\n  Trying critical-path-aware scheduling with {safety_buffer} day buffer...")

                    self.task_schedule = {}
                    self._critical_path_cache = {}

                    # Schedule with critical path awareness
                    self.schedule_tasks_with_critical_path_awareness(
                        safety_buffer_days=safety_buffer,
                        silent_mode=True
                    )

                    # Check results
                    level_metrics = self.calculate_lateness_metrics()
                    max_lateness = max((m['lateness_days'] for m in level_metrics.values()
                                        if m['lateness_days'] < 999999), default=0)
                    variance = self.calculate_utilization_variance()

                    print(f"    Results: lateness={max_lateness} days, variance={variance:.2f}")

                    # Count task criticality distribution
                    critical_count = 0
                    buffer_count = 0
                    flexible_count = 0
                    for task_id in self.task_schedule:
                        criticality = self.classify_task_criticality(task_id)
                        if criticality == 'CRITICAL':
                            critical_count += 1
                        elif criticality == 'BUFFER':
                            buffer_count += 1
                    ...
                    print(
                        f"    Task distribution: {critical_count} critical, {buffer_count} buffer, {flexible_count} flexible")

                    # Only accept if it doesn't violate target lateness
                    if max_lateness <= target_lateness:
                        if variance < best_variance:
                            best_schedule = self.task_schedule.copy()
                            best_variance = variance
                            best_lateness = max_lateness
                            best_safety_buffer = safety_buffer
                            print(f"    ✓ New best variance: {variance:.2f}")
                        else:
                            print(f"    ✗ No improvement in variance")
                    else:
                        print(f"    ✗ Violates target lateness ({max_lateness} > {target_lateness})")

                # Use the best schedule found
                self.task_schedule = best_schedule
                best_max_lateness = best_lateness

                print(f"\n  Final critical-path-aware results:")
                print(f"    Best safety buffer: {best_safety_buffer} days")
                print(f"    Best variance achieved: {best_variance:.2f}")
                if baseline_variance > 0:
                    improvement = ((baseline_variance - best_variance) / baseline_variance * 100)
                    print(f"    Improvement vs baseline: {improvement:.1f}%")
                print(f"    Max lateness maintained: {best_lateness} days")

        # Generate final results
        if best_config:
            # Apply best configuration for final results
            for team, capacity in best_config['mechanic'].items():
                self.team_capacity[team] = capacity
            for team, capacity in best_config['quality'].items():
                self.quality_team_capacity[team] = capacity

            # If we didn't do critical path scheduling, regenerate schedule
            if not enable_critical_path_scheduling:
                self.task_schedule = {}
                self._critical_path_cache = {}
                priority_list = self.generate_global_priority_list(allow_late_delivery=True, silent_mode=True)
            else:
                # Re-generate priority list with existing schedule
                priority_list = self.generate_global_priority_list(allow_late_delivery=True, silent_mode=True)
            ...
            return {
                ...
                'critical_path_scheduling_enabled': enable_critical_path_scheduling,
                'utilization_variance': best_variance if enable_critical_path_scheduling else None
            }
        else:
